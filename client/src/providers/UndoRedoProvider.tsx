...
export const UndoRedoProvider = ({children}: any) => {
    ...
    const addUndo = (lastBlocks: Block[], lastIndex: number) => {
        undoBlocksHistory.current.push(lastBlocks)
        undoFocusHistory.current.push(lastIndex)
    ...
    return (
        <UndoRedoContext.Provider value={{addUndo, readUndo, readRedo}} >
            {children}
